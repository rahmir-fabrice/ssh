...
// ORIGINALLY https://github.com/gliderlabs/ssh
...
type Server struct {
	...
	UserConns  map[string]map[*gossh.ServerConn]struct{} // patched to keep track of usernames of logged in 
...
func (srv *Server) Close() error {
	...
	for user, conns := range srv.UserConns { // patch for username disconnect
		for c := range conns {
			c.Close()
			delete(conns, c)
		}
...
// closes connections for a given username
func (srv *Server) CloseConnectionsForUser(user string) error {
	srv.mu.Lock()
	defer srv.mu.Unlock()
	if srv.UserConns[user] == nil {
		return errors.New("user not connected")
	}
	var conn_closed bool = false
	for conn := range srv.UserConns[user] {
		conn.Close()
		conn_closed = true
	}
	if(conn_closed == false) {
		return errors.New("user not connected")
	} else {
		return nil
	}
...
// return a list of usernames connected to ssh server (can have multiple connections per username)
func (srv *Server) GetConnectionsUsers() []string {
	var tmp_logged_in_users []string
	srv.mu.Lock()
	defer srv.mu.Unlock()
	for user, _ := range srv.UserConns { // patch for username disconnect
		tmp_logged_in_users = append(tmp_logged_in_users, user)
	}
	return tmp_logged_in_users
}

// returns number of connections (is total connected clients number)
func (srv *Server) GetConnectionsQTY() int {
	var tmp_conns_qty int = 0
	srv.mu.Lock()
	defer srv.mu.Unlock()

	for _, conns := range srv.UserConns { // patch for username disconnect
		for _ = range conns {
			tmp_conns_qty = tmp_conns_qty + 1
		}
	}

	// tmp_conns_qty = len(srv.UserConns) 
	return tmp_conns_qty
}


type SSH_SERVER_CONN_INFO struct {
	USER              string   `json:"user"`   // the user logged in
	ADDRS            []string  `json:"addrs"`   // the remote addresses connected in
}

// return a list of usernames and connected ip address , connected to ssh server (can have multiple connections per username)
func (srv *Server) GetConnections() []SSH_SERVER_CONN_INFO {
	var tmp_conn_infos []SSH_SERVER_CONN_INFO
	srv.mu.Lock()
	defer srv.mu.Unlock()
	for user, conns := range srv.UserConns { // patch for username disconnect
		var tmp_user_addresses []string = []string{}
		for c := range conns { // ServerConn.Conn
			tmp_user_addresses = append(tmp_user_addresses, c.RemoteAddr().String())
		}
		var tmp_conn_info SSH_SERVER_CONN_INFO
		tmp_conn_info.USER = user
		tmp_conn_info.ADDRS = tmp_user_addresses
		tmp_conn_infos = append(tmp_conn_infos, tmp_conn_info)
	}
	return tmp_conn_infos
}
